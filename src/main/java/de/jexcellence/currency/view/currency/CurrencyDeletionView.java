...
public class CurrencyDeletionView extends APaginatedView<Currency> {
	...
	protected @NotNull CompletableFuture<List<Currency>> getAsyncPaginationSource(
		...
		return this.jeCurrency.get(renderContext).getCurrencyRepository().findAllAsync(
			1,
			128
		);
	}
	...
	protected void renderEntry(
		...
		final JECurrency jeCurrency = this.jeCurrency.get(renderContext);
		
		final List<UserCurrency> userCurrencies = jeCurrency.getUserCurrencyRepository().findListByAttributes(
	...
	private void handleCurrencyDeletion(
		...
		final JECurrency jeCurrency = this.jeCurrency.get(clickContext);
	 ...
	 * @param jeCurrency the main plugin instance, must not be null
	...
	private void executeCurrencyDeletion(
		...
		CompletableFuture.supplyAsync(() -> {
			try {
				...
				final List<UserCurrency> userCurrencies = jeCurrency.getUserCurrencyRepository()
				...
				if (userCurrencies != null && !userCurrencies.isEmpty()) {
					for (UserCurrency userCurrency : userCurrencies) {
						jeCurrency.getUserCurrencyRepository().delete(userCurrency.getId());
					}
					jeCurrency.getLogger().info(
				...
				final List<CurrencyLog> currencyLogs = jeCurrency.getCurrencyLogRepository()
				                                                  .findByCriteria(
					                                                  null, // any log type
					                                                  null, // any log level
					                                                  null, // any player
				...
				if (!currencyLogs.isEmpty()) {
					for (CurrencyLog log : currencyLogs) {
						...
						jeCurrency.getCurrencyLogRepository().update(log);
						updatedLogCount++;
					}
					jeCurrency.getLogger().info(
				...
				boolean deletionSuccessful = jeCurrency.getCurrencyRepository().delete(currencyToDelete.getId());
				
				if (deletionSuccessful) {
					// Step 4: Remove from in-memory cache
					jeCurrency.getCurrencies().remove(currencyToDelete.getId());
					return true;
				} else {
