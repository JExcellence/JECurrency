...
public class CDeposit extends ServerCommand {
    ...
    protected void onPlayerInvocation(
        ...
        double depositAmount = this.doubleParameter(args, 2);
        ...
        this.jeCurrency.getCurrencyAdapter().getUserCurrency(targetPlayer, currencyIdentifier).thenAcceptAsync(
                usercurrency -> {
                    if (usercurrency == null) {
                        jeCurrency.getPlatformLogger().logDebug("PPlayerCurrency with player uuid: " + targetPlayer.getUniqueId() + " and currencies name: " + currencyIdentifier + " not found. (amount: " + depositAmount + ").");
                        return;
                    }

                    if (! usercurrency.deposit(depositAmount)) {
                        jeCurrency.getPlatformLogger().logDebug("Could not deposit the amount of " + depositAmount + " to the player uuid: " + targetPlayer.getUniqueId() + ".");
                        return;
                    }
